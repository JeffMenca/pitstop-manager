// src/pages/supplier/SupplierHome.jsx
import { useEffect, useMemo, useState, useCallback } from "react";
import { useNavigate } from "react-router-dom";
import { api } from "../../services/api";

const PATHS = {
  pedidos: ["/pedidoproveedor", "/pedidoProveedor", "/pedido", "/ordencompra"],
  estadosPedido: ["/estadopedido", "/estadoPedido", "/estadoordenreparacion"],
...
function estadoBadge(id) {
  ...
  if (n === 3) return "badge badge-success"; // Confirmado
  if (n === 4) return "badge badge-info";    // En curso
  if (n === 5) return "badge badge-info";    // En tránsito
  if (n === 6) return "badge badge-success"; // Entregado
  if (n === 7) return "badge badge-success"; // Aprobado
...
export default function SupplierHome() {
  ...
  const fetchAll = useCallback(async () => {
    ...
    setErr(null);
    try {
      const [ped, est, cot] = await Promise.all([
        tryGet(PATHS.pedidos),
        tryGet(PATHS.estadosPedido),
        tryGet(PATHS.cotizaciones),
      ]);
      setPedidos(Array.isArray(ped.data) ? ped.data : []);
      setEstadosPedido(Array.isArray(est.data) ? est.data : []);
      setCotizaciones(Array.isArray(cot.data) ? cot.data : []);
      if (!ped.ok) setErr("No pude obtener los pedidos (ajusta PATHS.pedidos).");
    } catch (e) {
  ...
  const pedidosActivos = useMemo(
    () => pedidos.filter(p => ![2, 6].includes(Number(p.estado))), 
    [pedidos]
  );
  const cotizPendientes = useMemo(
    () => cotizaciones.filter(c => !Boolean(c.confirmada ?? c.aprobada)),
    [cotizaciones]
  );
  ...
  return (
    <section className="prose max-w-none px-6 md:px-10">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
        ...
        <CardStat label="Cotizaciones pendientes" value={cotizPendientes.length} hint="Por confirmar" />
      ...
      <div className="flex gap-2 mb-6">
        ...
        <button className="btn btn-outline" onClick={()=>nav("/proveedor/cotizaciones")}>Ver cotizaciones</button>
          ...
          <tbody>
            {(pedidos.slice(0,5)).map(p => (
              <tr key={p.id}>
                ...
                <td>
                  <span className={estadoBadge(p.estado)}>{estadoTexto(p.estado)}</span>
                </td>
                <td>{p.total != null ? `$${fmt(p.total)}` : "—"}</td>
                <td className="text-right">
                  <button className="btn btn-sm btn-outline" onClick={()=>nav(`/proveedor/pedidos?focus=${p.id}`)}>Abrir</button>
                </td>
              </tr>
            ))}
            {pedidos.length === 0 && (
